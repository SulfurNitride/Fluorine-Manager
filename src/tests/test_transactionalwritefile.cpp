...
TEST(TransactionalWriteFile, ReadsTheAuthenticatedOriginalBeforeReplacement)
{
  QTemporaryDir temporary;
  ASSERT_TRUE(temporary.isValid());
  const QString existing = temporary.filePath("existing.txt");
  const QString missing = temporary.filePath("missing.txt");
  ASSERT_TRUE(writeBytes(existing, "old-generation"));

  MOBase::TransactionalWriteFile replace(existing);
  QByteArray contents;
  bool present = false;
  ASSERT_TRUE(replace.readOriginal(contents, present))
      << qPrintable(replace.errorString());
  EXPECT_TRUE(present);
  EXPECT_EQ(contents, "old-generation");
  ASSERT_TRUE(replace.replaceWith("new-generation"))
      << qPrintable(replace.errorString());

  MOBase::TransactionalWriteFile create(missing);
  ASSERT_TRUE(create.readOriginal(contents, present))
      << qPrintable(create.errorString());
  EXPECT_FALSE(present);
  EXPECT_TRUE(contents.isEmpty());
  ASSERT_TRUE(create.replaceWith("created")) << qPrintable(create.errorString());
}
...
TEST(TransactionalWriteFile, PublishesRequestedPermissionsWithTheNewGeneration)
{
  QTemporaryDir temporary;
  ASSERT_TRUE(temporary.isValid());
  const QString path = temporary.filePath("state.txt");
  ...
  MOBase::TransactionalWriteFile file(path);
  const auto requested = QFileDevice::ReadOwner | QFileDevice::WriteOwner;
  ASSERT_TRUE(file.setPermissions(requested)) << qPrintable(file.errorString());
  ASSERT_TRUE(file.replaceWith("new")) << qPrintable(file.errorString());
  EXPECT_EQ(readBytes(path), "new");
  EXPECT_TRUE(QFileInfo(path).permissions().testFlag(QFileDevice::WriteOwner));
}

TEST(TransactionalWriteFile, ReplacesReadOnlyLeafAndPreservesItsPermissions)
{
  QTemporaryDir temporary;
  ASSERT_TRUE(temporary.isValid());
  const QString path = temporary.filePath("state.txt");
  ...
  ASSERT_TRUE(file.readPermissions(originalPermissions))
  ...
  EXPECT_EQ(status.st_mode & (S_IWUSR | S_IWGRP | S_IWOTH), 0);
#else
  EXPECT_FALSE(QFileInfo(path).permissions().testAnyFlags(
...
TEST(TransactionalWriteFile, RejectsUnsafeSymlinksWithoutFollowingThem)
{
  ...
  QByteArray contents;
  bool present = false;
  EXPECT_FALSE(externalLink.readOriginal(contents, present));
...
TEST(TransactionalWriteFile, RejectsDirectoryAndFifoWithoutBlocking)
{
  ...
  QByteArray contents;
  bool present = false;
  EXPECT_FALSE(fifoFile.readOriginal(contents, present));
