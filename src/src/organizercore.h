...
private:
  using SignalAboutToRunApplication = boost::signals2::signal<
      bool(const QString &, const QDir &, const QString &), SignalCombinerAnd>;
  using SignalFinishedRunApplication =
      boost::signals2::signal<void(const QString &, unsigned int)>;
  using SignalUserInterfaceInitialized =
      boost::signals2::signal<void(QMainWindow *)>;
  using SignalProfileCreated =
      boost::signals2::signal<void(MOBase::IProfile *)>;
  using SignalProfileRenamed = boost::signals2::signal<void(
      MOBase::IProfile *, QString const &, QString const &)>;
  ...
  using SignalPluginEnabled =
      boost::signals2::signal<void(const MOBase::IPlugin *)>;

public:
  /**
   * Small holder for the game content returned by the ModDataContent feature
   * (the list of all possible contents, not the per-mod content).
   */
  struct ModDataContentHolder {
     ...
     *
     * @return the content with the given ID, or a null pointer if it is not
     * found.
     */
    const Content *findById(int id) const {
      auto it = std::find_if(
          std::begin(m_Contents), std::end(m_Contents),
          [&id](auto const &content) { return content.id() == id; });
     ...
     * @param includeFilter true to also apply the function to filter-only
    ...
    /**
     * @brief Apply fnIn to each content whose ID is in the given set, and fnOut
     * to each content not in the given set, excluding filter-only content (from
     * both cases) unless includeFilter is true.
     *
     * @param ids The set of content IDs.
  ...
  private:
    ...
    /**
     * @brief Construct a ModDataContentHolder without any contents (e.g., if
...
public:
  ...
  DirectoryRefresher *directoryRefresher() {
    return m_DirectoryRefresher.get();
  }
  ...
  getProfile(const QString &profileName) const;
  ...
  /**
   * @return the list of contents for the currently managed game, or an empty
   * vector if the game plugin does not implement the ModDataContent feature.
   */
  ...
  bool
  beforeRun(const QFileInfo &binary, const QDir &cwd, const QString &arguments,
            ...
            QString *usvfsManifestPath = nullptr,
            QString *usvfsLauncherPath = nullptr);
  ...
  void
  updateModsInDirectoryStructure(QMap<unsigned int, ModInfo::Ptr> modInfos);
  ...
  bool previewFile(QWidget *parent, const QString &originName,
                   const QString &path);
  ...
  void trackOverwriteMove(const QString &relativePath,
                          const QString &modFolderPath);

  void updateVFSParams(MOBase::log::Levels logLevel,
                       env::CoreDumpTypes coreDumpType,
                       const QString &coreDumpsPath,
...
public:
  ...
  findFileInfos(const QString &path,
                const std::function<bool(const MOBase::IOrganizer::FileInfo &)>
                    &filter) const;
  ...
  boost::signals2::connection
  onProfileRenamed(std::function<void(MOBase::IProfile *, QString const &,
  ...
  // - if immediateIfReady is true, the function will be called immediately if
  // no
...
private:
  ...
  doInstall(const QString &archivePath, MOBase::GuessedValue<QString> modName,
  ...
  void updateModsActiveState(const QList<unsigned int> &modIndices,
                             bool active);
...
private:
  ...
  MOBase::MemoizedLocked<std::shared_ptr<const MOBase::IFileTree>>
      m_VirtualFileTree;
