...
std::optional<QByteArray> readBounded(QFile& file, qint64 maximumSize)
{
...
bool restore(const QString& path, const Snapshot& state)
{
  if (state.existed) {
...
std::optional<QString> quarantineJournal(const QString& path)
{
...
QString nextRecoveryPath(const QString& path)
{
...
bool recoverFilesUnlocked(const QString& categoriesPath,
